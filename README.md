...
const ProductCard = ({ product, onRequestAccess, handleOpenDataQuality, id }) => {
  ...
  return (
    <div
      className={`border-2  w-full h-auto rounded-lg mb-4 p-6 flex flex-col 2xl:grid 2xl:grid-cols-2 gap-4 2xl:gap-2 ${
        isAccessible
          ? "bg-[#F0FDF466] border-[#0000001A]"
          : "bg-[#FECDCA1F] border-[#FBCE0799]"
      }`}
    >
      <div className={`w-full ${isAccessible !== "" ? "2xl:w-full" : "w-full"}`}>
        ...
        <p className="text-gray-600 text-xs mb-4">{product.table_description}</p>
      ...
      {isAccessible !== "" && (
        <div className="w-full pt-2 2xl:pt-0 2xl:pl-2 border-t 2xl:border-t-0 2xl:border-l border-[#00000029]">
          <div className="w-full flex items-center gap-2 mb-4">
            <div className="2xl:w-1/2 bg-white rounded-xl h-[68px] p-3">
          ...
          <div className="w-full flex items-center gap-4">
            {!isAccessible && isAccessible !== "" && (
              <button
                ...
                className="bg-yellow-400 hover:bg-yellow-500 text-gray-900 2xl:w-1/2 font-semibold px-2.5 py-2 text-xs rounded"
              >
                Request for Access
              </button>
            )}
            <button
              className="flex items-center 2xl:w-1/2 text-[10px] flex-start gap-1 text-gray-700 font-medium hover:text-gray-900 cursor-pointer"
